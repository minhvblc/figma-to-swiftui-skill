...
| **L2 static token trace (auto-emit + cross-ref against Figma artifacts)** | [figma-to-swiftui/references/verification-loop.md §4b](figma-to-swiftui/references/verification-loop.md) |
...
| Prototype wiring (button → screen edges + FigJam CONNECTOR arrows) → `prototype-wires.json` | [figma-to-swiftui/references/prototype-wires.md](figma-to-swiftui/references/prototype-wires.md), `scripts/c2-prototype-extract.sh` |
...
| **Convention probe (C1) — emits `c1-conventions.json` to drive C2/C3 routing** | `scripts/c1-probe.sh` |
...
Workflow là **Phase A → Phase B → Phase C, theo đúng thứ tự, có gate ở cuối mỗi phase**. Phase C bao gồm: C1 audit + C2 implement + C3 self-check (Pass 1/2/3/3b) + C4 copy assets + C5 build/screenshot/diff + C6 asset completeness + C7 no-system-chrome. Coding conventions (folder layout / ViewModel pattern / function size / IK-bridge routing) áp dụng tại C2 write-time qua reference docs — không có gate script riêng (xem table "Coding-conventions layer" phía dưới).
...
- Visual diff không được dùng từ "weasel" (`approximately`, `roughly`, `close enough`, `~`, `nearly`, ...) trong PASS rows.

---

## Coding-conventions layer (áp dụng tại C2 write-time)

C1 step **probe project conventions** rồi ghi `c1-conventions.json` via `scripts/c1-probe.sh`. C2 (Implement) đọc file này để khớp với project — folder layout, ViewModel pattern, có IKNavigation hay không, có IKMacros hay không, IKFont enum tên gì. Không có gate tự động — agent phải đọc reference docs và áp dụng đúng:

| Convention | Reference |
|---|---|
| Screens/<Name>Screen/<Name>Screen.swift, prefix subview | `figma-to-swiftui/references/project-structure.md` |
| @MainActor + enum Action + send(_:) reducer + flat @Published | `figma-to-swiftui/references/viewmodel-pattern.md` |
| Function ≤ 50 dòng, golden path, modifier order, [weak self], custom Error enum | `figma-to-swiftui/references/swift-style.md` |
| IKNavigation (chỉ khi project đã dùng) | `figma-to-swiftui/references/iknavigation-bridge.md` |
| IKMacros (chỉ khi project đã dùng) | `figma-to-swiftui/references/ikmacro-bridge.md` |
| IKFont/AppFont enum (chỉ khi project có) | `figma-to-swiftui/references/fonts-styling-bridge.md` |
...
**Driver scripts (Tier 1 speed wins):** ngoài các gate cũ (C5/C6/C7), repo có driver scripts gộp công đoạn:

| Script | Thay thế việc agent gõ tay | Phase |
|---|---|---|
| `c1-probe.sh` | 11 grep/find của conventions probe | C1 |
...
| `c2-prototype-extract.sh` | Parse `metadata.json` → `prototype-wires.json` (button → screen edges + FigJam CONNECTOR arrows, screen-FRAME attribution) | Gate A |
| `c5-capture.sh` | simctl screenshot + sips shrink (cmp pair) | C5.5 + C5.5b |
| `c3-static-checks.sh` | Pass 3 + 3b bash sweep | C3 |
| `c3-token-trace.sh` | **L2 static token trace** — cross-refs `c2-audit.json` × tokens/design-context/metadata/manifest, emits `c3-trace.md` + `c3-trace.json` | C3 (L2) |
| `c3-driver.sh` | **C3 orchestrator** — subcommands `trace` / `residual` / `ssim` / `aggregate`. `aggregate` emits `c3-gate.json` with final GATE | C3 |
| `figma-audit-src/` | SwiftSyntax Swift package built locally → binary at `~/.local/share/figma-audit/bin/figma-audit`. Parses `*Screen.swift`/`*View.swift` and emits `c2-audit.json` rows (color/font/padding/frame/image/text/stack). Driven by PostToolUse hook `figma-to-swiftui-audit-emit.sh`. | C2 write-time (L1) |
| `c5-engine-select.sh` | Probe Engine A (xcode MCP) vs Engine B (xcodebuild) | C5 |
| `c6-asset-completeness.sh` | Registry ↔ xcassets ↔ src completeness gate | C6 |
| `c7-no-system-chrome.sh` | Status-bar / home-indicator / screen-corner redraw scan | C7 |
| `colorset-codegen.sh` | Emit Asset Catalog colorsets from tokens.json | B0b |
| `timing-report.sh` | Đọc manifest.timing và in bảng wall-time | regression check |
| `timed-run.sh` | Helper của `timing-report.sh` — wrap command, record wall-time | per-step instrumentation |
...
| `mode-detect.sh` | Classify greenfield-vanilla / greenfield-ikame / brownfield-* | Phase 0 |
| `ikxcodegen-scaffold.sh` | Scaffold greenfield Ikame project via ikxcodegen | Phase 0 |
| `vanilla-scaffold.sh` | Scaffold greenfield vanilla SwiftUI project | Phase 0 |
| `xcodeproj-add-files.sh` | Append generated Swift files to .pbxproj | C2/C4 |
...
- Mọi script gate (C5/C6/C7) đều phải in `GATE: PASS`, `GATE: FAIL: <reason>`, hoặc `GATE: SKIP (<reason>)` ở cuối, exit code khớp.
...
|---|---|---|
| **Add** script `scripts/<new>.sh` | (a) `install.sh`: block `for src in "$SCRIPTS_SRC"/b0a-*.sh ...` (tìm bằng `grep -n 'SCRIPTS_SRC.*b0a' install.sh`); (b) `doctor.sh` §7 first list (tìm bằng `grep -n '^for script in c1-probe' doctor.sh`); (c) `doctor.sh` §7 second list dưới `INSTALLED_SCRIPTS_DIR`; (d) `doctor.sh` §9 drift glob (tìm bằng `grep -n 'SCRIPTS_REPO' doctor.sh`); (e) reference doc tham chiếu (SKILL.md hoặc `references/*.md`) nếu skill flow phải gọi script | `scripts/doctor.sh` in ra `<new>.sh present and executable` ở cả `$SCRIPTS_DIR` và `$INSTALLED_SCRIPTS_DIR`; `All N of N checks passed` |
...
- Driver script khác orchestrates sub-scripts
