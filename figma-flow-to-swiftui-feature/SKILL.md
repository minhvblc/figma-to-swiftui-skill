...
> **Script path resolution.** Every `scripts/X.sh` reference below is `~/.claude/scripts/X.sh` when running from an iOS project (the cwd you actually work in). The repo-relative form is what the skill source uses; `scripts/install.sh` copies all helpers to `~/.claude/scripts/` so they are reachable wherever you invoke the skill. Same applies to `../figma-to-swiftui/references/*.md` which install to `~/.claude/skills/figma-to-swiftui/references/`. When in doubt: `ls ~/.claude/scripts/`, or `bash ~/.claude/scripts/doctor.sh`.
...
When the document conflicts with Figma: doc decides behavior (transitions, validation, state); Figma decides structure (layout, copy, visual hierarchy).
...
- **Navigation audit (mandatory):** confirm the routing uses `NavigationStack` / `NavigationSplitView` (not deprecated `NavigationView`) and that destinations register via `.navigationDestination(for:)` not `NavigationLink(destination:)`. If the existing codebase **mixes** the two, **STOP** — flag to the user before writing new screens; mixing breaks navigation. **NOTE:** when the convention probe (next bullet) sets `usesIKNavigation = true`, this check inverts — the mandatory state is IKNavigation throughout, NOT vanilla NavigationStack; see `../figma-to-swiftui/references/iknavigation-bridge.md`.
- **Observation audit (mandatory):** confirm shared state uses `@Observable` + `@MainActor` (iOS 17+) or `ObservableObject` + `@Published` + `@StateObject` (iOS 16 fallback) consistently — not mixed. If project baseline is iOS 16+, the legacy form is mandatory; document that decision in the screen graph output.
- **Token enum audit (mandatory):** locate token enums by canonical and alternative names — `Spacing` / `AppSpacing` / `Padding`, `IKFont` / `AppFont` / `Typography`, `IKCoreApp` / `AppColors` / `ColorPalette`. List their cases. C2 routes Figma values through whichever is detected (see `scripts/c1-probe.sh`). When a project has none of these, the skill falls back to inline literals + `@ScaledMetric` — do NOT introduce a new enum file unless the user explicitly asks.
- **Coding-conventions probe (mandatory, flow-shared).** Run the convention probe ONCE for the entire flow via `bash ~/.claude/scripts/c1-probe.sh <projectFolder>`. Write the result to `.figma-cache/_shared/c1-conventions.json`. Every per-screen Phase C reads this same file — do not re-probe per screen. Resolved fields the screen graph (Step 3) must respect:
  ...
  - **Mode detection** — at flow start, run `bash ~/.claude/scripts/mode-detect.sh <projectFolder> --write-cache` to classify as `greenfield-ikame | greenfield-vanilla | brownfield-ikame | brownfield-vanilla | ambiguous`. Greenfield-Ikame requires `ikxcodegen` scaffold before Phase A (ASK USER first) — run `scripts/ikxcodegen-scaffold.sh <Name>`. The skill must NOT generate raw `.xcodeproj` for greenfield Ikame; always go through `ikxcodegen`. Ambiguous mode → STOP and ask the user before scaffolding over existing files; persist `userConfirmed: true` into `mode.json` when OK'd.
...
**Ikame projects (`usesIKCoreApp == true`) — mandatory placement rules.** When Step 2 detected the Ikame umbrella, shared scaffolding goes to the canonical Ikame topology:
...
- **App-wide models** — `<entitiesPath>/<source>/<entitiesPrefix><Domain>Model.swift`. Prefix is project-specific. New models that the flow uniquely needs go here — flow-only models stay in `Screens/<Feature>/Models/`.
- **Shared services** — `Core/Services/<Name>Service.swift` when the service is consumed by ≥2 features. A service initially scoped to 1 feature lives in `Screens/<Feature>/ViewModel/`. **MUST promote** the moment a 2nd feature consumes it.
- **Shared subcomponents** — `Components/App<Name>View.swift` (cross-cutting reusable, App prefix) or `Components/<Name>View.swift` (domain-specific). Multi-file components use folder form `Components/<Name>View/`.
...
- **Structural rules apply:** each screen view in its own file; sub-sections > ~40 lines extract into separate `View` structs (in their own files), not computed properties returning `some View`. See [`../figma-to-swiftui/references/project-structure.md`](../figma-to-swiftui/references/project-structure.md) and [`../figma-to-swiftui/references/swift-style.md`](../figma-to-swiftui/references/swift-style.md).
...
A flow is NOT done while any screen has neither. **A successful build alone is not C5** — regardless of engine, you also need a real render + visual compare. On Engine A (xcode MCP — default on Xcode 26+): `mcp__xcode__BuildProject` + `mcp__xcode__RenderPreview` + the C5.6 6-step procedure. On Engine B (xcodebuild + simctl fallback): `xcodebuild build` + `simctl boot/install/launch` + `simctl io screenshot` + C5.6. If you only built and stopped, you have NOT run C5; the Done-Gate is violated and you must complete C5 before declaring done.
