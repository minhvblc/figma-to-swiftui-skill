...
- **Token enum audit (mandatory):** locate token enums by canonical and alternative names — `Spacing` / `AppSpacing` / `Padding`, `IKFont` / `AppFont` / `Typography`, `IKCoreApp` / `AppColors` / `ColorPalette`. List their cases. C2 routes Figma values through whichever is detected per `../figma-to-swiftui/references/adaptation-workflow.md` §0. When a project has none of these, the skill falls back to inline literals + `@ScaledMetric` — do NOT introduce a new enum file unless the user explicitly asks.
