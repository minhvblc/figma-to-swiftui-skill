...
- **Coding-conventions probe (mandatory, flow-shared).** Run the convention probe ONCE for the entire flow per [`../figma-to-swiftui/references/adaptation-workflow.md` §0](../figma-to-swiftui/references/adaptation-workflow.md#0-convention-probe-mandatory-run-before-the-audit). Write the result to `.figma-cache/_shared/c1-conventions.json`. Every per-screen Phase C reads this same file — do not re-probe per screen. Resolved fields the screen graph (Step 3) must respect:
  - `screenFolderConvention` — drives where each new `*Screen.swift` / `*ViewModel.swift` lands. Three modes: `screen-based` (`Screens/<Name>Screen/<Name>Screen.swift`), `flat`, `ikame-feature-flat` (`Screens/<Feature>/<Name>Screen.swift` + `ViewModel/`, `Subviews/`, `Models/` subfolder). See [`../figma-to-swiftui/references/project-structure.md`](../figma-to-swiftui/references/project-structure.md).
  - `viewModelPattern` — every new ViewModel uses State + Action + `send(_:)` reducer. Two flavors: `state-action-reducer` (canonical, `@Published var route: Route?`) and `state-action-route-publisher` (Ikame, `let routePublisher = PassthroughSubject<Route, Never>()`). See [`../figma-to-swiftui/references/viewmodel-pattern.md`](../figma-to-swiftui/references/viewmodel-pattern.md) §1 + §1b.
  - `usesIKNavigation` (+ `routerName` + `navigationItemEnumName` + `navigationItemPath`) — when true, the flow EXTENDS the existing `<routerName>` and adds new cases to `<navigationItemEnumName>`; it does NOT introduce vanilla `NavigationStack` or invent a parallel router. See `../figma-to-swiftui/references/iknavigation-bridge.md` §5.
  ...
  - **Ikame cascade flags** (set when `usesIKCoreApp == true`, i.e. project's Podfile has `pod 'IKCoreApp'` OR any Swift file imports it):
    - `usesIKPopup` — popups go through `await IKPopup.shared.showPopup(...)` per [`../figma-to-swiftui/references/ikpopup-bridge.md`](../figma-to-swiftui/references/ikpopup-bridge.md). Vanilla `.sheet` / `.alert` require `// allow-vanilla-popup: <reason>` justification (D-507).
...
- **Routes** — extend `<navigationItemPath>` (typically `Core/Router/Main/MainRoute.swift`) with new `<navigationItemEnumName>.<case>` entries; extend the corresponding `MainRouter` (or `<routerName>`) `makeView(from:)` switch. Do NOT create a parallel route enum or per-feature router unless the user explicitly asks.
