...
NVIOL=${#VIOLATIONS[@]}
if [ "$NMISS" -eq 0 ] && [ "$NVIOL" -eq 0 ]; then
  echo "${C_GRN}PASS${C_RST}: $TOTAL_REG registry assets match xcassets; 0 systemName violations."
  exit 0
fi
echo "${C_RED}FAIL${C_RST}: $NMISS missing assets, $NVIOL systemName violations"
exit 1
