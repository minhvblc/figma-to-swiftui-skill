...
Materialization happens in B0b: `scripts/colorset-codegen.sh` reads `tokens.json` (light/dark hex pairs from Figma) and emits colorsets into the project's xcassets. The Ikame branch of the codegen names new colorsets `color<HEX>`; the generic branch uses the semantic name from tokens.json.
