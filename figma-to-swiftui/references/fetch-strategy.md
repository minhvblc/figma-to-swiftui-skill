...
- **Auth / 401-403 (any tool, including `figma_extract_tokens`)** → STOP the entire Phase A and surface to user; this is not a per-screen issue. **Single exception:** `figma_extract_tokens` returning HTTP 403 with the Figma `message` field containing the exact substring `"requires the file_variables:read scope"` triggers the plan-limit fallback per "Token-extract fallback rule" below — fallback allowed with mandatory disclosure protocol. Every OTHER 401/403 = STOP, no fallback, no exceptions. Do **NOT** treat any other 403 as "Variables API not available" — that 200-empty-with-warnings case is distinct from the 403-scope-not-on-PAT case which is itself distinct from a generic 403. Confusing them leads to silently proceeding without Variables data when the real fix is a token re-issue or workspace correction. STOP, show the verbatim error to the user, link to `references/mcpfigma-setup.md` §Troubleshooting.
...
| **Plan-gated Variables scope (`file_variables:read` missing on PAT)** | HTTP 403, `message` contains the exact substring `"requires the file_variables:read scope"`. Two indistinguishable sub-cases: plan-gated (Free / Professional / Organization without Enterprise — scope not in PAT settings UI) or scope-omitted (plan exposes the scope but PAT generated without ticking it). | Fall back to inline tokens **with mandatory disclosure**. Write `tokens.json` with `_note: "reconstructed from inline styles — file_variables:read scope missing on PAT (Figma 403). Plan-gated for non-Enterprise; user-fixable if scope option visible in Figma settings."`. The Verification summary MUST surface the verbatim 403 `message` field AND the `Variables source: inline-fallback (...)` line per [`figma-to-swiftui/SKILL.md` §"MCPFigma edge cases"](../SKILL.md). This is the ONLY 403 that allows fallback. |
| **Other permission failure** | HTTP 401 (`unauthorized`), or HTTP 403 with any other `message` (file access denied, workspace permission, missing `file_content:read`, etc.). | **STOP**. Do NOT fall back. Surface verbatim to the user with the fix path: re-issue PAT at https://figma.com/settings with all scopes the plan exposes (especially `File content: Read` and `Variables: Read` if visible), and verify the file is in the token-owner's workspace. See `references/mcpfigma-setup.md` §Troubleshooting. |

The same three-outcome split applies to `figma_build_registry` and `figma_export_assets_unified`, but **without** the plan-limit-fallback exception — those endpoints have no plan-gated scope. Any `forbidden` on registry / asset export is always a STOP, never a fallback trigger.
